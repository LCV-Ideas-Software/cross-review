...
export function statusInstruction(): string {
  return [
    ...
    "Field length budget: keep `summary` SHORT (max 800 chars). READY uses its canonical sentence; NOT_READY uses only canonical BLOCKER lines; NEEDS_EVIDENCE uses one tight paragraph.",
    ...
    "Canonical field labels are case-sensitive. Use exactly `Checklist-Item:`, `Location:`, `Attachment:`, `sha256=`, and `Artifact quote:` with the casing shown here; lowercase lookalikes are invalid.",
    "After JSON decoding, READY items contain these three lines in this exact order (encode the two line breaks as `\\n` in raw JSON):",
    'Attachment: <persisted-path>\nsha256=<64 lowercase hex>\nArtifact quote: "<literal text from that same attachment>"',
    "For NOT_READY, `summary` must contain exactly one `BLOCKER: <path:line>` line per finding and no other text. Each blocker has exactly one source item that prepends `Location: <the same path:line>`; use backticks around the complete path:line when the path contains spaces. Put remediation in caller_requests. To close an owned evidence request, `Checklist-Item: <id>` may precede Location/Attachment. No other prefix or suffix prose is allowed.",
    ...
    "When the prompt supplies `Reviewed Artifact Custody`, every definitive READY or NOT_READY decision must include a canonical source whose Attachment path and sha256 exactly match that block and whose Artifact quote is one substantive line visible in the reviewed-artifact excerpt. A legacy unbound artifact quote is not definitive custody.",
    "READY requires the complete reviewed artifact to have been visible in the provider call. If its custody block says the view is a prefix, use NEEDS_EVIDENCE. NOT_READY may cite a blocker from a truncated view only when the canonical quoted blocker line is inside the visible prefix.",
    "For a non-patch reviewed draft, NOT_READY BLOCKER/Location uses the persisted reviewed-artifact path and physical line. A fully visible Git-validated unified-diff draft may instead use an exact logical post-image path and line. If that diff is prefix-only, logical post-image locations are invalid; use a complete visible physical artifact line or NEEDS_EVIDENCE.",
    ...
    "NOT_READY is also a definitive factual verdict: every canonical BLOCKER line must have exactly one Location-matched canonical literal evidence source. If you cannot cite the blocker from the reviewed artifact or a persisted attachment, use NEEDS_EVIDENCE and put the exact missing proof in `caller_requests`; malformed or uncited NOT_READY will be downgraded automatically.",

...
export function mergeCost(costs: Array<CostEstimate | undefined>): CostEstimate {
  ...
  for (const cost of costs) {
    ...
    if (cost?.cache_storage_cost != null) {
      cacheStorage += cost.cache_storage_cost;
      // Round 10: an explicit zero is a marker (deterministically free
      // creation), not absence - the merge keeps the line when any input
      // carried it.
      cacheStorageKnown = true;
    }
  ...
  if (cacheStorageKnown) merged.cache_storage_cost = cacheStorage;

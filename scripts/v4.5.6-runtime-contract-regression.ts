...
const regressions: Regression[] = [
  ...
  },
  {
    ...
    run: async () => {
      ...
      // v06.00.00 pricing hard block: the configured pin is priced only by a card
      ...
      const familyOnlyFlattened = flattenFileConfigToEnvMap({
        ...
        model_cost_rates: {
          claude: {
            ...
            "claude-fable-9": { input_per_million: 10, output_per_million: 50 },
          },
        },
      } as never);
      assert.equal(
        ...
        undefined,
        "central-config flattening must not price claude-fable-9-1 with a claude-fable-9 family card (synthetic family)",
      );
      const exactFlattened = flattenFileConfigToEnvMap({
        models: { claude: "claude-fable-9-1" },
        model_cost_rates: {
          claude: {
            "claude-fable-9": { input_per_million: 1, output_per_million: 2 },
            "claude-fable-9-1": { input_per_million: 10, output_per_million: 50 },
          },
        },
      } as never);
      assert.equal(
  ...
  },
  {
    ...
    run: async () => {
      ...
      const config: AppConfig = {
        ...base,
        // v06.00.00: pin the canonical GPT-6 Astra id explicitly so the family

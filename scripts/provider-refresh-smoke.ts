...
// v06.00.00: this suite reads the hard-coded defaults, not the operator's
...
assert.equal(
  config.models.codex,
  "gpt-6-astra",
  "v06.00.00: the default OpenAI pin must be gpt-6-astra when no central config or env override is present",
);
assert.equal(
  config.models.claude,
  "claude-fable-5-1",
  "v06.00.00: the default Anthropic pin must be claude-fable-5-1 when no central config or env override is present",
);
...

{
  ...
  const familyMatrix: Array<{
    ...
    {
      // v06.00.00 canonical pin: GPT-6 Astra rejects `none`; the official
...
{
  // v06.00.00: the canonical OpenAI pin is GPT-6 Astra. The selector keeps the
...
{
  // v06.00.00 / issue #271: the full Claude Fable 5.1 incompatibility audit on

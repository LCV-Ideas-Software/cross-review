...
function assertNativeCodeqlEnforcement(workflow) {
  assert.match(
    workflow,
    /^ {8}uses: github\/codeql-action\/analyze@[0-9a-f]{40} # v\d+\.\d+\.\d+$/m,
    "CodeQL must use the official analyzer pinned to an immutable commit",
  );
  assert.match(
    workflow,
    /^ {6}security-events: write # uploads the CodeQL SARIF analyses$/m,
    "CodeQL must retain the permission required for the native upload",
  );
  assert.doesNotMatch(
    workflow,
    /LCV-Ideas-Software\/\.github\/codeql-sarif-gate|Enforce zero CodeQL findings|CODEQL_RESULTS:|mapfile\s+-d|find\s+"\$CODEQL_RESULTS"/,
    "CodeQL enforcement must remain native to the repository ruleset without a custom or cross-repository SARIF gate",
  );
}

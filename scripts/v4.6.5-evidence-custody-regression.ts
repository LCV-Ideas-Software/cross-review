...
const regressions: Regression[] = [
  ...
  {
    name: "a JSON-compatible quoted Git path with spaces preserves its basename custody",
    run: () => {
      const preflight = evidencePreflight({
        task: "Review the attached dependency update.",
  ...
  {
    name: "a Git C-style octal path outside the JSON-compatible subset fails closed",
    run: () => {
      const preflight = evidencePreflight({
        task: "Review the attached dependency update.",
      ...
      assert.equal(preflight.pass, false);
      assert.deepEqual(preflight.unattached_evidence_references, ["report.md"]);
    },
  },
  {

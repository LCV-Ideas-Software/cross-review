...
1. um agente ou host MCP apresenta uma tarefa e um rascunho;
...
| Versão preparada pelo source  | `v07.00.00`                        |
| Data do source/release target | `08/09/2026`                       |
...
sendo responsabilidade do agente que submete o caso.
...
| Caller                 | Quem submete a tarefa ao `cross-review`. É um dos agentes reconhecidos.                                                           |
...
`claude-fable-5-1` é o pin Anthropic canônico. O request omite o campo explícito
...
| `session_finalize`                      | Encerra a própria sessão não terminal como `aborted` (peticionário persistido); `converged` só pelo runtime.                               |
...
- `server_info` expõe readiness, peers habilitados e estado de tokens sem expor
  segredos;
- seis capability tokens vinculam hosts às seis identidades de agente; havia um
  sétimo, para uma identidade `operator` cujo token deveria morar num console
  humano separado — esse host não existe, porque toda a superfície é MCP e é
  exercida por agentes, então o segredo não se ligava a ninguém e saiu junto
  com as ferramentas que o exigiam;
- evidência inline/estruturada de caller autenticado é persistida e transportada
  automaticamente como `caller_submitted_unverified`, que é a única procedência
  que um anexo carrega; o fechamento da própria sessão como `aborted` cabe ao
  peticionário persistido;
...
Tokens de caller reforçam essa separação. Cada peer recebe apenas seu token.
Havia um sétimo, `operator`, exigido pelas ferramentas privilegiadas e destinado
a um console humano dedicado. Esse console nunca existiu — toda a superfície é
MCP e é exercida por agentes —, então o token não se ligava a host nenhum e saiu
junto com as ferramentas que o pediam. A geração acontece no boot; distribuir
os arquivos é ato humano no disco, fora do MCP.
...
- anexos opcionais via `session_attach_evidence`, abertos a qualquer peer e sem
  promoção alguma: o artefato carrega a mesma procedência do campo `evidence`,
  que segue sendo o caminho normal;
- logs relevantes.
...
revalidação estrita, sem fechar pedido alheio ou estado terminal. Não existe
promoção de evidência: não há degrau acima de `caller_submitted_unverified`.
...
| ---------------- | ------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `v07.00.00`      | 08/09/2026    | Correção: uma falha de polling do Perplexity deixa de recriar a run que acabou de abandonar. O laço pede ao provedor que pare antes de propagar, mas o cancelamento é reconhecido de forma assíncrona (`status: "cancelling"`) e todo desfecho é engolido; no instante em que o laço de retry reentra no closure a run está, na melhor hipótese, se encerrando e ainda cobrando — e o closure contém o create. O rethrow agora carrega `safe_to_repeat: false`, lido só pelo laço de retry (issue #298).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
| `v06.00.00`      | 08/09/2026    | Versão maior: as cinco chaves legadas do cartão de preço da Sonar (`request_fee_low_per_1000`, `request_fee_medium_per_1000`, `request_fee_high_per_1000`, `citation_tokens_per_million`, `deep_research_reasoning_tokens_per_million`) voltam a ser rejeitadas pelo schema estrito da configuração central — remova qualquer cartão ou chave Sonar do `config.json` antes de atualizar, senão o arquivo inteiro é ignorado e as chamadas pagas ficam bloqueadas com `CROSS_REVIEW_CONFIG_FILE_INVALID`. `TokenUsage.citation_tokens`, os três line items de `CostEstimate` e as cinco chaves legadas de `CostRateConfig` saem das declarações publicadas, então um consumidor que as referencie deixa de compilar; junto com elas saem `server_info.config_load.deprecated_keys_ignored` e a cláusula de `citation_tokens` do predicado de trabalho do provedor. Além disso: as chamadas longas do Perplexity (revisor e relator) passam ao modo background (`background: true` mais `GET /v1/agent/{id}`), porque o provedor encerrava a conexão síncrona por volta de 300 s; essas duas requisições passam a enviar `store: true`, então o Perplexity as retém (issue #296). |

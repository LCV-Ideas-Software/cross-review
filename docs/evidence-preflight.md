...
a peer alias that no captured current occurrence consumed (a fragmented id
such as "gpt five six sol", or a denial that names no token) cannot be judged
lexically and is ALWAYS reported as `unsupported_current_state_claim` with one
instruction: restate with the exact contiguous model id, splitting mixed
current/planning/historical sentences. There is deliberately no
evidence-corroboration channel for such claims — the restated claim is what
gets judged by value against the configured pins. Negated or instructional examples

...
- **does NOT** execute a shell, run `git diff`, apply a patch, or read the repo;
...
From v4.6.5 onward, when submitted evidence itself contains a unified diff,
cross-review delegates patch grammar and pathname decoding to the official Git
executable through the non-applying
`git apply --numstat --summary -z --whitespace=nowarn -p1 -` reporting
operation. The parser runs with `shell: false` outside the repository and
receives only the caller-supplied bytes on standard input. Custody is granted
only to unambiguous, non-deleted post-image paths reported by Git; case,
punctuation and quoted path bytes remain exact. If Git is unavailable, rejects
the patch, times out, or emits unsafe output, patch-derived custody fails
closed. Human `BEGIN FILE` envelopes remain a separate explicit convention.

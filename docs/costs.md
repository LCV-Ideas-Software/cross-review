...
`deep_research_reasoning_tokens_per_million`) on any card
(`cost_rates.<peer>` or `model_cost_rates.<peer>.<model>`) exactly like any
other unknown key. During 5.x they were deprecated no-ops, accepted and
stripped; the matching `CostRateConfig` members, `TokenUsage.citation_tokens`
and the `CostEstimate` line items `request_cost`, `citation_tokens_cost` and
`deep_research_reasoning_tokens_cost` left the shipped declarations in
v06.00.00 as scheduled, together with the `mergeUsage` / `mergeCost`
passthrough; sessions persisted before v05.00.00 keep their stored
`total_cost`. A rejected key is reported the same way as any other: the boot notice and

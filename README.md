...
| -------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| **`v04.06.05`**      | **Unreleased source target.** Binds reviewed artifacts, visible attachment slices, and each persisted redacted provider-prompt variant to durable custody; adds coherent official-Git post-image validation, typed round kinds, exactly-once circular recovery, and no-clobber terminal-mirror repair. |
...
| **`v03.03.00`**      | Minor — Caller peer-selection lock (operator directive 12/05/2026: "TODOS OS AGENTES/PEERS SEMPRE PARTICIPAM, INDEPENDENTE DA ESCOLHA OU VONTADE DO CALLER").                                                                                                                                          |
...
For `NOT_READY`, `summary` contains exactly one `BLOCKER: <path:line>` line per
finding and no other text. Each blocker has exactly one source item: the
optional `Checklist-Item` line, when applicable, comes first; mandatory
`Location: <the same path:line>` comes next; then `Attachment`, `sha256`, and
the terminal `Artifact quote` follow in that order. Remediation belongs in
`caller_requests`, not in the summary or citation suffix.
...
When supplied evidence contains a unified diff, cross-review uses a fixed,
argument-constant
`git -c core.quotePath=false apply --numstat --summary -z --whitespace=nowarn -p1 -`
parse to derive exact post-image paths and candidate hunk regions. It then
requires monotonic, non-overlapping old/new ranges, equal unchanged gaps and
global destination uniqueness: each effective destination may appear in at
most one section. Git never applies the patch, no caller command is executed,
no caller repository is selected or mutated, and the process runs without a
shell or user Git configuration.
The official Git contract makes the reporting flags turn off application unless
`--apply` is explicitly present; cross-review never passes it. LF and CRLF patch
terminators map to canonical line content, while a bare CR, contradictory hunk
geometry, or a repeated destination fails closed. The executable is accepted
only from fixed platform-standard installation paths. Git is an optional runtime capability:
if it is absent, times out, rejects the patch, or emits an unsafe path, only
patch-derived custody fails closed; ordinary text and `BEGIN FILE` evidence
remain available. `server_info.evidence_patch_validation` and
`runtime_capabilities.evidence_patch_validation` return the same five-field
readback: `available`, `version`, `provenance`, `failure_reason`, and the exact
fixed `operation`. They never expose the executable path.
